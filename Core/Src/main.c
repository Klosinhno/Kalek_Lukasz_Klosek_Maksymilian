...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : PI + BH1750 + ADC + LCD (Wartości z przecinkiem)
  ******************************************************************************
  */
...
/* ===== INCLUDES ===== */
#include "main.h"
#include "adc.h"       // Twoja biblioteka do ADC
#include "i2c.h"       // I2C1 (wygenerowane przez CubeMX)
#include "lcd_i2c.h"   // Twoja biblioteka do LCD (I2C2)
#include "tim.h"       // PWM
...
char UART_Cmd[] = "X000";
uint8_t UART_Len;
uint8_t RangeError = 0;
int Mode = 1; // 1 = Tryb PI, 0 = Manual

/* ===== PROTOTYPY FUNKCJI ===== */
void SystemClock_Config(void);
void MX_I2C2_Init(void); // Funkcja inicjalizująca I2C2 dla LCD
...
int main(void)
{
  ...
  uint32_t LastDisplayTick = 0; // Czas ostatniego odświeżenia ekranu

  /* Stałe czasowe */
  const uint32_t ControlPeriod = 100; // Regulacja co 100ms (10Hz)
  const uint32_t DisplayPeriod = 250; // Wyświetlanie co 250ms (4Hz) - czytelniej dla oka
  ...
  while (1)
  {
    ...
    /* To wykonuje się dokladnie co 100ms (+/- jitter rzędu 1ms) */
    if (CurrentTick - LastControlTick >= ControlPeriod)
    {
        ...
        if (Mode == 1)
        {
           // Teraz dt jest rzeczywiście bliskie 0.1s
    ...
    /* To wykonuje się rzadziej, żeby nie spowalniać procesora */
