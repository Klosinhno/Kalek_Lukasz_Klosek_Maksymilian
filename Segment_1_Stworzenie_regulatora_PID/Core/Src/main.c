...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : PI + BH1750 (Tylko UART, Kp=0.1, Ki=0.1)
  ******************************************************************************
  */
...
PI_HandleTypeDef hpi = {
  .Kp = 0.1f,         // Wzmocnienie proporcjonalne (zmienione na 0.1)
  .Ki = 0.1f,         // Wzmocnienie całkujące (zmienione na 0.1)
  .Setpoint = 100.0f, // Domyślna wartość zadana
  .Integral = 0.0f,
  .OutMin = 0.0f,
  .OutMax = 100.0f
};
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef* huart)
{
    if (huart == &huart3)
    {
        ...
        if (UART_Cmd[0] == 'D') { // Direct PWM
            ...
            // Opcjonalnie: Zerowanie całki przy zmianie celu
            // hpi.Integral = 0.0f; 
        }
...
int main(void)
{
    ...
    /* Stałe czasowe */
    const uint32_t ControlPeriod = 100; // 100ms
    const uint32_t DisplayPeriod = 250; // 250ms
    ...
    while (1)
    {
        ...
        if (CurrentTick - LastDisplayTick >= DisplayPeriod)
        {
            ...
            // Wyświetlanie: Zadana | Zmierzona | Wyjście PWM
